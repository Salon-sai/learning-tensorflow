...
class SDNE(object):
    ...
    def __init__(self, graph, hidden_size=[32, 16], alpha=1e-6, beta=5., nu1=1e-5, nu2=1e-4):
        ...
        self._embeddings = {}
        self.model, self.emb_model = self.def_model_graph(hidden_size=hidden_size)
        self.model.compile("adam", [self.loss_1(self.alpha), self.loss_2(self.beta)])
    ...
    def def_model_graph(self, l1=1e-5, l2=1e-4, hidden_size=[256, 128]):
        a = Input((self.node_size))
        l = Input((self.node_size))
        output = a
