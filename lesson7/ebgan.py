    ...
    with slim.arg_scope([slim.conv2d_transpose],
        ...
        with slim.arg_scope([slim.batch_norm], is_training=train_phase, decay=0.9, epsilon=1e-5,
                            param_initializers={
                                ...
                                "gamma": tf.random_normal_initializer(mean=1, stddev=0.03)
                            }):
            weight = tf.get_variable('Generator/W', [z_dim, 2 * IMAGE_SIZE * IMAGE_SIZE], initializer=tf.truncated_normal_initializer(stddev=0.03))
